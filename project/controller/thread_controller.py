...
@thread_router.post("")
def post_thread(thread_request_dto: ThreadCreateRequestDTO, jwt: str = Cookie(...)):
...
@thread_router.patch("/{id}")
def edit_thread(id: int, thread_edit_request_dto: ThreadEditRequestDTO, jwt: str = Cookie(...)):
    thread_response_dto = thread_service.edit_thread(id=id, thread_edit_request_dto=thread_edit_request_dto, jwt=jwt)
    return JSONResponse(content=thread_response_dto.as_dict(), status_code=200)
